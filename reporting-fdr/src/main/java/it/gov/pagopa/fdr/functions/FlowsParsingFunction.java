...
public class FlowsParsingFunction {
  ...
  /**
   * This function will be invoked when a new or updated blob is detected at the specified path. The
   * blob contents are provided as input to this function.
   */
  @FunctionName("FlowsParsingFunction")
  public void run(
      @BlobTrigger(
              name = "BlobXmlTrigger",
              path = "%FLOWS_XML_BLOB%/{name}",
              dataType = "binary",
              connection = "FLOW_SA_CONNECTION_STRING")
          byte[] content,
    ...
    try {
      ...
      String dataOraFlusso =
          flowInfo[5].substring(0, flowInfo[5].length() - 4); // remove extension file
      log.info(
          "Processing flow PSP {} flow {} with date {}",
          identificativoPSP,
          identificativoFlusso,
          dataOraFlusso);
      this.optionsService.optionsProcessing(
          handler.getIdentificativoUnivocoRegolamento(),
          handler.getDataRegolamento(),
          handler.getOptions(),
          identificativoPSP,
